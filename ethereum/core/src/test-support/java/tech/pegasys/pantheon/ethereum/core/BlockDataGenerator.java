...
public class BlockDataGenerator {
  ...
  public BlockHeader header(final long blockNumber, final BlockOptions options) {
    return header(blockNumber, BlockBody.empty(), options);
  }
  ...
  public BlockBody body(final BlockOptions options) {
    final List<BlockHeader> defaultOmmers = new ArrayList<>();
    final int ommerCount = random.nextInt(3);
    for (int i = 0; i < ommerCount; i++) {
      defaultOmmers.add(header());
    }
    final List<Transaction> defaultTxs = new ArrayList<>();
    defaultTxs.add(transaction());
    defaultTxs.add(transaction());

    return new BlockBody(options.getTransactions(defaultTxs), options.getOmmers(defaultOmmers));
  }
  ...
  public static class BlockOptions {
    ...
    public BlockOptions setLogsBloomFilter(final LogsBloomFilter logsBloom) {
      this.logsBloom = Optional.of(logsBloom);
      return this;
    }

    public BlockOptions addOmmer(final BlockHeader... ommer) {
      if (!ommers.isPresent()) {
        ommers = Optional.of(new ArrayList<>());
      }
      ommers.get().addAll(Arrays.asList(ommer));
      return this;
    }
  }
}
